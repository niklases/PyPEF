...
from os import mkdir, PathLike, environ, path
import pickle
import numpy as np
...
# Detach comp. graph explanantion, first .detach()  , then  .cpu():
# https://stackoverflow.com/questions/63582590/why-do-we-call-detach-before-calling-numpy-on-a-pytorch-tensor

# 
# /home/niklas/Data/dev/contrastive_learning_test/DMS_ProteinGym_substitutions/BLAT_ECOLX_Jacquier_2013.csv
#df = pd.read_csv(path.join(path.dirname(__file__), 'DMS_ProteinGym_substitutions/BLAT_ECOLX_Jacquier_2013.csv'))  # 'DMS_ProteinGym_substitutions/HSP82_YEAST_Mishra_2016.csv'))
#ms_, seqs_, y_trues_ = df['mutant'].to_numpy(), df['mutated_sequence'].to_numpy(), df['DMS_score'].to_numpy()
#print(df)
...
class GREMLIN:
    ...
    def opt_adam_step(self, lr=1.0, b1=0.9, b2=0.999, b_fix=False):
        ...
        #if var_list is None:
        #    var_list = model.parameters()   #?!
        #gradients = tf.gradients(loss, var_list)
        #v_and_w_flat = torch.cat((self.v.flatten(), self.w.flatten()), 0)
        #print(v_and_w_flat.dtype, v_and_w_flat.size(), len(v_and_w_flat))
        #print(f'Estimates number of bits for {len(v_and_w_flat)} floats:\n'
        #      f'= {len(v_and_w_flat)} * 32 = {len(v_and_w_flat)*32} Bits = {int(len(v_and_w_flat)*32/8)} Bytes '
        #      f'= {int(len(v_and_w_flat)*32/8)//1000} KB = {int(len(v_and_w_flat)*32/8)//1000000} MB '
        #      f'= {int(len(v_and_w_flat)*32/8)/1000000000:.1f} GB')
        #if b_fix:
        #    t = tf.Variable(0.0, "t")
        #opt = []
        #v = torch.Tensor(v, requires_grad=True)
        #w = torch.Tensor(w, requires_grad=True)
        #gradients = torch.gradient(loss, v+w?!)
        self.v.retain_grad()
        self.w.retain_grad()
        loss = self.loss(self.v, self.w)
        loss.backward()
        #print('v.grad.size():', self.v.grad.size())
        #print('w.grad.size():', self.w.grad.size())
        #for n, (x, g) in enumerate(zip(self.v, self.v.grad)):
        #print(n,x.sum(), x.size(), g.sum(), g.size())
        ...
        #self.w = self.w.add(-lr_tmp * mt_tmp)
        self.vt_v = vt_tmp_v
        self.mt_v = mt_tmp_v
        self.vt_w = vt_tmp_w
        self.mt_w = mt_tmp_w
        ...
        #        # get mt and vt from prior adam opt steps
        ...
        #
        #        opt.append(x.assign_add(-lr_tmp * mt_tmp))   # Update (add to) x 
        #        opt.append(vt.assign(vt_tmp))                # Update vt
        #        opt.append(mt.assign(mt_tmp))                # Update mt
        #
        #if b_fix:
    ...
    def sym_w(self, w):
        ...
        w = w * torch.reshape(1 - torch.eye(x), (x, 1, x, 1)).to(self.device)  # Is this slow?
        #print(w.size())
        #w_tf = w + tf.transpose(w, [2, 3, 0, 1])
        #print(w_tf.shape)
    ...
    def loss(self, v, w):
        ...
        #msa = torch.Tensor(self.msa_trimmed).long().to(self.device)  # <--
        #print(msa.size())

        # one-hot encode msa
        #oh_msa = torch.nn.functional.one_hot(msa, self.states).to(torch.float32).to(self.device)   # <--
        #print(oh_msa.size())

        # msa weights
        #msa_weights = tf.compat.v1.placeholder(tf.float32, shape=(None,), name="msa_weights")
        #msa_weights = torch.zeros_like(torch.from_numpy(self.msa_weights))
        #msa_weights = torch.from_numpy(self.msa_weights).to(torch.float32).to(self.device)   # <--
        ...
        #print('vw TF shape:', vw.shape)

        #print('vw size 1', torch.tensordot(oh_msa, w, dims=2).size())
        vw = v + torch.tensordot(self.oh_msa, w, dims=2)
        #print('vw size:', vw.size())
    ...
    def run_optimization(self, opt_rate=1.0, batch_size=None):
        ...
        #msa_cat1 = tf.keras.utils.to_categorical(self.msa_trimmed, self.states)
        ...
        #y_pred = self.get_score(seqs=seqs_)
        #print('DCA Spearman untrained:', round(spearmanr(y_trues_, y_pred)[0], 3))
        ...
        print('Initial loss:', round(initial_loss.detach().cpu().numpy() * self.n_eff, 2))
        i = 0
        while i < self.opt_iter:
            self.opt_adam_step()
            #y_pred = self.get_score(seqs=seqs_)
            if (i + 1) % 10 == 0:
                print(i+1, 'LOSS:', round(self.loss(self.v, self.w).detach().cpu().numpy() * self.n_eff, 2))  # , 'SpearmanR:', round(spearmanr(y_trues_, y_pred)[0], 3))
            i += 1
...
def save_gremlin_as_pickle(alignment: str, wt_seq: str, opt_iter: int = 100):
    ...
    """
    print(f'Inferring GREMLIN DCA parameters based on the provided MSA...')
    ...
    print(f'Saving GREMLIN model as Pickle file...')
    pickle.dump(
        {
            'model': gremlin,
...
if __name__ == '__main__':
    import os
    GREMLIN(os.path.join(os.path.dirname(__file__), 'DMS_msa_files/HSP82_YEAST_2023-08-07_b01.a2m'))  # HSP82_YEAST_full_11-26-2021_b01.a2m
