...
def plot_performance(mut_data, plot_name, mut_sep=':'):
    ...
    for i, (dset_key, dset_paths) in enumerate(mut_data.items()):
        ...
        if c > 0:
            print(f'{int(c)} of {len(var_pos)} ({ratio_input_vars_at_gaps * 100:.2f}%) input '
        ...
        hybrid_perfs = []
        for train_size in [25, 50, 75, 100, 200]:
            try:
                ...
                train_test_size_texts.append(plt.text(
                    n_tested_datasets, abs(spearmanr(y_test, y_test_pred)[0]), f'{len(y_train)}'  + r'$\rightarrow$' + f'{len(y_test)}', size=4))
            except ValueError:
    ...
    plt.plot(range(len(tested_dsets)), np.full(np.shape(tested_dsets), np.nanmean(dset_dca_perfs)), color='tab:blue', linestyle='--')
    plt.text(len(tested_dsets) - 1, np.nanmean(dset_dca_perfs), f'{np.nanmean(dset_dca_perfs):.2f}')

    plt.plot(range(len(tested_dsets)), np.array(dset_hybrid_perfs)[:, 0], 'o--', markersize=8, color='tab:orange', label='Hybrid (25)')
    plt.plot(range(len(tested_dsets)), np.full(np.shape(tested_dsets), np.nanmean(np.array(dset_hybrid_perfs)[:, 0])), color='tab:orange', linestyle='--')
    plt.text(len(tested_dsets) - 1, np.nanmean(np.array(dset_hybrid_perfs)[:, 0]), f'{np.nanmean(np.array(dset_hybrid_perfs)[:, 0]):.2f}')

    plt.plot(range(len(tested_dsets)), np.array(dset_hybrid_perfs)[:, 1], 'o--', markersize=8, color='tab:green', label='Hybrid (50)')
    plt.plot(range(len(tested_dsets)), np.full(np.shape(tested_dsets), np.nanmean(np.array(dset_hybrid_perfs)[:, 1])), color='tab:green', linestyle='--')
    plt.text(len(tested_dsets) - 1, np.nanmean(np.array(dset_hybrid_perfs)[:, 1]), f'{np.nanmean(np.array(dset_hybrid_perfs)[:, 1]):.2f}')

    plt.plot(range(len(tested_dsets)), np.array(dset_hybrid_perfs)[:, 2], 'o--', markersize=8, color='tab:red',  label='Hybrid (75)')
    plt.plot(range(len(tested_dsets)), np.full(np.shape(tested_dsets), np.nanmean(np.array(dset_hybrid_perfs)[:, 2])), color='tab:red', linestyle='--')
    plt.text(len(tested_dsets) - 1, np.nanmean(np.array(dset_hybrid_perfs)[:, 2]), f'{np.nanmean(np.array(dset_hybrid_perfs)[:, 2]):.2f}')

    plt.plot(range(len(tested_dsets)), np.array(dset_hybrid_perfs)[:, 3], 'o--', markersize=8, color='tab:purple', label='Hybrid (100)')
    plt.plot(range(len(tested_dsets)), np.full(np.shape(tested_dsets), np.nanmean(np.array(dset_hybrid_perfs)[:, 3])), color='tab:purple', linestyle='--')
    plt.text(len(tested_dsets) - 1, np.nanmean(np.array(dset_hybrid_perfs)[:, 3]), f'{np.nanmean(np.array(dset_hybrid_perfs)[:, 3]):.2f}')

    plt.plot(range(len(tested_dsets)), np.array(dset_hybrid_perfs)[:, 4], 'o--', markersize=8, color='tab:brown', label='Hybrid (200)') 
    plt.plot(range(len(tested_dsets)), np.full(np.shape(tested_dsets), np.nanmean(np.array(dset_hybrid_perfs)[:, 4])), color='tab:brown', linestyle='--')
    plt.text(len(tested_dsets) - 1, np.nanmean(np.array(dset_hybrid_perfs)[:, 4]), f'{np.nanmean(np.array(dset_hybrid_perfs)[:, 4]):.2f}')
    ...
    plt.legend()
    plt.tight_layout()
    adjust_text(train_test_size_texts, only_move={'points': 'y', 'text': 'y'}, force_points=0.5)
    plt.ylim(0.0, 1.0)
